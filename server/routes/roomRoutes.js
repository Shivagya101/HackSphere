...
console.log("Room routes file loaded, functions available:", {
...
router.get("/github/repos", async (req, res) => {
  try {
    ...
    if (!user || !user.githubAccessToken) {
      return res.status(401).json({ error: "No GitHub access token for user" });
    }
    // Debug log: print which user and token are being used
    console.log(
    ...
    if (!response.ok) {
      if (response.status === 401) {
        return res.status(400).json({
          error:
            "GitHub access not configured. Please enter repository URL manually.",
          useCustomUrl: true,
        });
      }
    ...
    res.status(500).json({
      error:
        "Failed to fetch GitHub repositories. Please enter repository URL manually.",
      useCustomUrl: true,
    });
  }
});
...
router.post("/github/create-repo", async (req, res) => {
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 422) {
        return res.status(422).json({
          error:
            "Repository name already exists or is invalid. Please choose a different name.",
    ...
    res.status(500).json({
      error:
        "Failed to create GitHub repository. Please try again or use a manual URL.",
      useCustomUrl: true,
    });
  }
});
...
router.get("/github/branches/:repoUrl", async (req, res) => {
  try {
    ...
    // Only unauthenticated request for public repos
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        return res
          .status(401)
          .json({ error: "GitHub access token is invalid or expired" });
      }
...
router.get("/github/commits/:repoUrl/:branch", async (req, res) => {
  try {
    ...
    // Remove .git suffix if present
    const repo = repoRaw.endsWith(".git") ? repoRaw.slice(0, -4) : repoRaw;
    const apiUrl = `https://api.github.com/repos/${owner}/${repo}/commits?sha=${branch}&per_page=5`;
    // Only unauthenticated request for public repos
    let response = await fetch(apiUrl, {
      headers: {
        Accept: "application/vnd.github.v3+json",
...
router.get("/github/commits/:repoUrl", async (req, res) => {
  try {
    ...
    // Fetch commits from GitHub API (default branch) using user's token
    const decoded = jwt.verify(token, process.env.JWT_SECRET);
    const user = await User.findById(decoded.userId);
    if (!user || !user.githubAccessToken) {
      return res.status(401).json({ error: "No GitHub access token for user" });
