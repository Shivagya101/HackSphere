...
function RoomHistory() {
  ...
  const fetchRoomHistory = async (userId) => {
    try {
      ...
      const response = await fetch(`http://localhost:3000/user/${userId}/rooms`);
  ...
  const deleteRoomFromHistory = async (roomId) => {
    try {
      const response = await fetch(`http://localhost:3000/user/${user.id}/rooms/${roomId}`, {
        method: 'DELETE',
        headers: {
      ...
      <div className="bg-white rounded-lg shadow-xl p-8 max-w-2xl w-full">
        ...
        <div className="mb-4">
          ...
          <p className="text-sm text-gray-500 mt-1">
            Here are the rooms you've joined. Click on a room ID to copy it, or click "Join" to go back to home with that room ID.
          </p>
        </div>
        ...
        {roomHistory.length === 0 ? (
          <div className="text-center py-8">
            ...
            <h3 className="text-lg font-semibold text-gray-600 mb-2">No Rooms Yet</h3>
            <p className="text-gray-500">You haven't joined any rooms yet. Create or join a room to see it here!</p>
          </div>
        ) : (
          <div className="space-y-3">
            {roomHistory.map((room, index) => (
              <div key={index} className="border rounded-lg p-4 hover:bg-gray-50 transition duration-200">
                <div className="flex justify-between items-center">
                  <div className="flex-1">
                    ...
                    <div className="text-sm text-gray-500 mt-1">
                      ...
                      {room.lastVisited && room.lastVisited !== room.joinedAt && (
                        <span className="ml-2">• Last visited: {formatDate(room.lastVisited)}</span>
                      )}
                    </div>
                  </div>
                  <div className="flex space-x-2">
                    ...
                    <button
                      onClick={() => {
                        if (confirm(`Are you sure you want to remove room ${room.roomId} from your history?`)) {
        ...
        <div className="mt-6 p-4 bg-blue-50 rounded-lg">
          ...
          <ul className="text-sm text-blue-700 space-y-1">
            <li>• <strong>Copy Room ID:</strong> Click the copy icon next to any room ID</li>
            <li>• <strong>Join Room:</strong> Click "Join" to go back to home with that room ID pre-filled</li>
            <li>• <strong>Remember Password:</strong> You'll need to enter the room password when joining</li>
          </ul>
        </div>
      </div>
    </div>
